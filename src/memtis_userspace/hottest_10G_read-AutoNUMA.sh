...
function func_main() {
    ...
    if [ -n "${DRAM_SIZE}" ]; then
        mkdir -p ${result_dir}/run-memtis-10g-read/microbench/AutoNUMA/${DRAM_SIZE}
        LOG_DIR=${result_dir}/run-memtis-10g-read/microbench/AutoNUMA/${DRAM_SIZE}
    else
        mkdir -p ${result_dir}/run-memtis-10g-read/microbench/AutoNUMA
        LOG_DIR=${result_dir}/run-memtis-10g-read/microbench/AutoNUMA
    fi
