...
# 使用方法:
#   1) 仅指定 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-nomad.sh 10G
#   2) 同时指定监控间隔(秒) 与 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-nomad.sh 1 10G
# 参数解析规则:
#   - 若第一个参数为纯数字，则视为监控间隔(秒)，第二个参数为 DRAM 大小（可选）
#   - 若仅传入一个非数字参数，则视为 DRAM 大小，监控间隔默认 1 秒

# 整合脚本：使用 perf-record.sh 运行 YCSB 基准测试并记录内存访问
# 用法: ./ycsb-run-benchmark-with-monitor-nomad.sh [DRAM_SIZE]
# 示例: ./ycsb-run-benchmark-with-monitor-nomad.sh 10G
...
BENCHMARK_SCRIPT="$SCRIPT_DIR/../hottest_10G_read-nomad.sh"
...
if [ $BENCHMARK_EXIT_CODE -eq 0 ]; then
    ...
    # perf 相关的文件（CSV, perf.data）会输出到 results/nomad/ 目录
else
    echo "基准测试执行失败"
fi
