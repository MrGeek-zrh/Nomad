...
# 使用方法:
#   1) 仅指定 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-AutoNUMA-no-perf.sh 10G
#   2) 同时指定监控间隔(秒) 与 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-AutoNUMA-no-perf.sh 1 10G
# 参数解析规则:
#   - 若第一个参数为纯数字，则视为监控间隔(秒)，第二个参数为 DRAM 大小（可选）
#   - 若仅传入一个非数字参数，则视为 DRAM 大小，监控间隔默认 1 秒

# 整合脚本：运行 YCSB 基准测试
# 用法: ./ycsb-run-benchmark-with-monitor-AutoNUMA-no-perf.sh [DRAM_SIZE]
# 示例: ./ycsb-run-benchmark-with-monitor-AutoNUMA-no-perf.sh 10G
...
if [ -n "$DRAM_SIZE_ARG" ]; then
    cd "$WORKING_DIR" && sudo env DRAM_SIZE=$DRAM_SIZE_ARG "$BENCHMARK_SCRIPT"
else
    cd "$WORKING_DIR" && sudo env DRAM_SIZE= "$BENCHMARK_SCRIPT"
fi
...
if [ $BENCHMARK_EXIT_CODE -eq 0 ]; then
    ...
    # 注意：原有的输出（output.log, before_vmstat.log, after_vmstat.log 等）会输出到 results/AutoNUMA/ 目录
else
    echo "基准测试执行失败"
fi
