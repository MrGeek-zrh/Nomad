...
# 使用方法:
#   1) 仅指定 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-AutoNUMA.sh 10G
#   2) 同时指定监控间隔(秒) 与 DRAM 大小
#      ./ycsb-run-benchmark-with-monitor-AutoNUMA.sh 1 10G
# 参数解析规则:
#   - 若第一个参数为纯数字，则视为监控间隔(秒)，第二个参数为 DRAM 大小（可选）
#   - 若仅传入一个非数字参数，则视为 DRAM 大小，监控间隔默认 1 秒

# 整合脚本：使用 perf-record.sh 运行 YCSB 基准测试并记录内存访问
# 用法: ./ycsb-run-benchmark-with-monitor-AutoNUMA.sh [DRAM_SIZE]
# 示例: ./ycsb-run-benchmark-with-monitor-AutoNUMA.sh 10G
...
PERF_RECORD_SCRIPT="$HOME/hmm/bench_dir/perf-record.sh"
BENCHMARK_SCRIPT="$SCRIPT_DIR/../hottest_10G_read-AutoNUMA.sh"
...
# CSV 输出文件名
CSV_OUTPUT="$RESULT_BASE/ycsb-zipfian-${CSV_SUFFIX}.csv"
RESULT_LOG="$RESULT_BASE/zipfan_hottest_10G.read.log"

# 检查必要文件是否存在
missing_components=()

if [ ! -f "$PERF_RECORD_SCRIPT" ]; then
    missing_components+=("perf-record.sh ($PERF_RECORD_SCRIPT)")
fi
...
echo "=========================================="
echo "开始运行 YCSB 基准测试（使用 perf-record.sh）"
echo "=========================================="
echo "perf-record.sh 脚本: $PERF_RECORD_SCRIPT"
echo "基准测试脚本: $BENCHMARK_SCRIPT"
echo "DRAM 大小: ${DRAM_SIZE_ARG:-未指定}"
echo "输出路径: $CSV_OUTPUT"
echo "=========================================="
...
# 使用 perf-record.sh 运行基准测试
# --in-cmd 参数需要完整的命令字符串
# --out 参数使用完整路径
echo "正在启动 perf 记录..."
echo "命令: $PERF_RECORD_SCRIPT --in-cmd \"$RUN_CMD\" --out \"$CSV_OUTPUT\""
echo "=========================================="

if "$PERF_RECORD_SCRIPT" --in-cmd "$RUN_CMD" --out "$CSV_OUTPUT"; then
...
if [ $BENCHMARK_EXIT_CODE -eq 0 ]; then
    echo "基准测试执行成功"
    echo "输出文件: $CSV_OUTPUT"
    if [ -f "$CSV_OUTPUT" ]; then
        echo "CSV 文件已生成"
    fi
    # 注意：原有的输出（output.log, before_vmstat.log, after_vmstat.log 等）会由 nomigrate 脚本输出到 results/nomigrate/ 目录
    # perf 相关的文件（CSV, perf.data）会输出到 results/AutoNUMA/ 目录
else
    echo "基准测试执行失败"
fi
